...
class DartCompleter( language_server_completer.LanguageServerCompleter ):
  ...
  def OnFileReadyToParse( self, request_data ):
    ...
    return super( DartCompleter, self ).OnFileReadyToParse( request_data )
    ...
    return responses.BuildDebugInfoResponse(
      ...
      servers = [
        responses.DebugInfoServer(
          name = "dart Dart Language Server",
  ...
  def ServerIsReady( self ):
    if not self.ServerIsHealthy():
      _logger.info('fatman: server is not healthy!')
      return False
    if self._received_ready_message.is_set():
      _logger.info('fatman: _received_ready_message is not set!')
      return False
    if not super(DartCompleter, self).ServerIsReady():
      _logger.info("fatman super is not ready!")
      return False
    return True
    # return ( self.ServerIsHealthy() and
    #          self._received_ready_message.is_set() and
    #          super( DartCompleter, self ).ServerIsReady() )
  ...
  def _StopServer( self ):
    _logger.info("fatman, call _StopServer")
    with self._server_state_mutex:
      _logger.info( 'Shutting down dart...' )
      ...
      if not self._ServerIsRunning():
        _logger.info( 'dart Language server not running' )
        self._CleanUp()
        return

      _logger.info( 'Stopping dart server with PID {0}'.format(
      ...
      try:
        ...
        _logger.info( 'dart Language server stopped' )
      except Exception:
        _logger.exception( 'Error while stopping dart server' )
  ...
  def GetCodepointForCompletionRequest( self, request_data ):
    ...
    # too many possible candidates for dart to return anything useful. This is
    # because we don't send the currently typed characters to dart. The
  ...
  def HandleNotificationInPollThread( self, notification ):
    ...
    if notification[ 'method' ] == 'language/status':
      ...
      if message_type == 'Started':
        _logger.info( 'dart initialized successfully.' )
        self._received_ready_message.set()

    super( DartCompleter, self ).HandleNotificationInPollThread( notification )
